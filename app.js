...
function Observer() {
  ...
  this.on('listUpdate', function (data) {
    listTags[0].update(data);
  });

  this.on('messageUpdate', function (text) {
    messageTags[0].update(text);
  });
}

},{"./tag/list.tag":7,"./tag/message.tag":8,"./tag/search.tag":9,"riot":2}],7:[function(require,module,exports){
var riot = require('riot');
module.exports = riot.tag('list', '<ul> <li each="{ opts.data }" class="listItem"> <span class="listItem__head">{id}</span> <span class="listItem__content"><a href="http://category.auctions.yahoo.co.jp/list/{id}/" class="link" target="_blank">{title}</a></span> </li> </ul>', function(opts) {var _this = this;

this.on('update', function (data) {
  if (data) _this.opts.data = data;
});
});

},{"riot":2}],8:[function(require,module,exports){
var riot = require('riot');
module.exports = riot.tag('message', '<div class="message"> <span name="messageText" class="message__text">{ opts.text }</span> </div>', function(opts) {var _this = this;

this.on('update', function (text) {
  if (text) {
    _this.opts.text = text;
    _this.trigger('effect');
  }
});

this.on('effect', function (e) {
  setTimeout(function () {
    _this.messageText.classList.remove('--show');
    setTimeout(function () {
      _this.messageText.classList.add('--show');
    }, 0);
  }, 0);
});
});

},{"riot":2}],9:[function(require,module,exports){
var riot = require('riot');
module.exports = riot.tag('search', '<input class="input__text +luminous" type="text" name="keyword" placeholder="search keyword">', function(opts) {var _this = this;
...
var keywordValue = function keywordValue() {
  ...
  }).skipDuplicates().filter(function (v) {
    return v.length > 2;
  }).toProperty();
};
...
loadingBus.onValue(function (keyword) {
...
keywordBus.onValue(function (keyword) {
  request.get('' + opts.uri + '/search/keyword/' + keyword).end(function (err, res) {
